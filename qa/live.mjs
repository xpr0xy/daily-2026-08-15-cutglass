...
let blogResult=null;for(let attempt=1;attempt<=3;attempt++){
 ...
 blogResult=await blog.evaluate(async()=>{const text=document.body.innerText;const img=[...document.images].find(i=>i.src.includes('/images/cutglass.png'));const original=await new Promise(resolve=>{const probe=new Image();probe.onload=()=>resolve({src:probe.src,w:probe.naturalWidth,h:probe.naturalHeight});probe.onerror=()=>resolve(null);probe.src=`/images/cutglass.png?v=${Date.now()}`});return {title:document.title,marker:text.includes('CUTGLASS: Paint the Gaps Into a Bassline'),appLink:[...document.links].some(a=>a.href.includes('daily-2026-08-15-cutglass')),image:original,renderedImage:img?{src:img.src,currentSrc:img.currentSrc,w:img.naturalWidth,h:img.naturalHeight}:null}}).catch(()=>null);await blog.close();
